...
use crate::input::from_input::{FromInput, FromInputImpl, Preflight};
use crate::input::Input;

pub use crate::codegen::LocalData;
...
/// # Example
///
/// The implementation of this trait is generated by using custom derive.
///
/// ```
...
#[derive(Debug)]
pub struct Local<T>(PhantomData<(fn() -> T, std::cell::UnsafeCell<()>)>);
    ...
    T: LocalData,
{
    ...
    pub fn replace_with(&mut self, other: T) -> T {
        self.with_mut(|value| std::mem::replace(value, other))
    }
}

impl<T> FromInput for Local<T> where T: LocalData {}
impl<T> FromInputImpl for Local<T>
where
    T: LocalData,
{
    type Error = Error;
    ...
    fn preflight(input: &mut Input<'_>) -> Result<Preflight<Self>, Error> {
        if input.locals().contains_key(&T::KEY) {
            Ok(Preflight::Completed(Local(PhantomData)))
        } else {
